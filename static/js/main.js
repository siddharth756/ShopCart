...
if (cart_obj == null || cart_obj == undefined) {
  ...
  cartLength = Object.keys(cart_obj).length;
}
document.getElementById("cart-badge").innerHTML = cartLength;
...
$(document).on("click", ".atc", function () {
  ...
  if (cart[item_id] != undefined) {
    ...
    cart[item_id][2] += parseFloat(
      document.getElementById("pp" + item_id).innerHTML
    );
  } else {
    quantity = 1;
...
function DisplayCart(cart) {
  ...
  for (var x in cart) {
    cartString +=
      cartIndex + " Name : " + cart[x][1] + " : Qty : " + cart[x][0] + "<br>";
    cartIndex += 1;
  }
  cartString +=
    '<a href="/checkout/" class="btn btn-warning mt-2">Checkout</a>';
